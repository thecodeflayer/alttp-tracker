...
export const staticMapDW = {
    ...
    brewery: {x: 163, y: 878,title: 'Brewery', desc: '', req:[],
        validate(items, dungeons){
        return items.bombs && items.moonpearl && regionHelper.northWestDW(items, dungeons);
    }},
    cShapeHouse: {x: 310, y: 726,title: 'C-Shaped House', desc: '', req:[],
        validate(items, dungeons){
        return items.moonpearl && regionHelper.northWestDW(items, dungeons);
    }},
    chestGame: {x: 76, y: 702,title: 'Chest Game', desc: '', req:[],
        validate(items, dungeons){
        return items.moonpearl && regionHelper.northWestDW(items, dungeons);
    }},
    ...
    blacksmith: {x: 223, y: 994,title: 'Blacksmith', desc: '', req:[],
        validate(items, dungeons){
        return items.mirror && items.moonpearl && items.glove === 2
            && regionHelper.northWestDW(items, dungeons);
    }},
    purpleChest: {x: 457, y: 802,title: 'Purple Chest', desc: '', req:[],
        validate(items, dungeons){
        return items.mirror && items.moonpearl && items.glove === 2
            && regionHelper.northWestDW(items, dungeons);
    }},
    hypeCave: {x: 896, y: 1168,title: 'Hype Cave', desc: '', req:[],
        validate(items, dungeons){
        return items.moonpearl && items.bombs && regionHelper.southDW(items, dungeons);
    }},
    stumpy: {x: 463, y: 1024,title: 'Stumpy', desc: '', req:[],
        validate(items, dungeons){
        return items.moonpearl && regionHelper.southDW(items, dungeons);
    }},
    diggingGame: {x: 85, y: 1043,title: 'Digging Game', desc: '', req:[],
        validate(items, dungeons){
        return items.moonpearl && regionHelper.southDW(items, dungeons);
    }},
    mireShed: {x: 59, y: 1205,title: 'Mire Shed', desc: '', req:[],
        validate(items, dungeons){
